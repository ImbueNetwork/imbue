...
fn brief_submit_deposit_below_minimum() {
    build_test_externality().execute_with(|| {
		...
		assert_noop!(BriefsMod::submit_brief_direct(RuntimeOrigin::signed(*ALICE), *TESTHASH, 100_000, d_below_minimum, CurrencyId::Native), Error::<Test>::DepositBelowMinimum);
	});
}
...
fn brief_submit_bounty_below_minumum() {
    build_test_externality().execute_with(|| {
		...
		assert_noop!(BriefsMod::submit_brief_direct(RuntimeOrigin::signed(*ALICE), *TESTHASH, b_below_minimum, d_above_minimum, CurrencyId::Native), Error::<Test>::BountyBelowMinimum);
	});
}
...
fn brief_submit_contribution_more_than_bounty() {
    build_test_externality().execute_with(|| {
		...
		assert_noop!(BriefsMod::submit_brief_direct(RuntimeOrigin::signed(*ALICE), *TESTHASH, b_above_minimum, d_above_bounty, CurrencyId::Native), Error::<Test>::ContributionMoreThanBounty);
	});
}
...
fn brief_submit_already_exists_in_block() {
    build_test_externality().execute_with(|| {
		...
		assert_ok!(BriefsMod::submit_brief_direct(RuntimeOrigin::signed(*ALICE), *TESTHASH, b_above_minimum, d_above_minimum, CurrencyId::Native));
		assert_noop!(BriefsMod::submit_brief_direct(RuntimeOrigin::signed(*ALICE), *TESTHASH, b_above_minimum, d_above_minimum, CurrencyId::Native), Error::<Test>::BriefAlreadyExists);
	});
}
