    ...
    build_test_externality().execute_with(|| {
        ...
        assert_noop!(
            Proposals::raise_dispute(
                RuntimeOrigin::signed(BOB),
                project_key,
                vec![1u32, 11u32].try_into().unwrap()
            ),
            Error::<Test>::MilestoneDoesNotExist
        );
    })
    ...
    build_test_externality().execute_with(|| {
        ...
        assert_noop!(
            Proposals::raise_dispute(
                RuntimeOrigin::signed(BOB),
                project_key,
                vec![submitted_milestone_key].try_into().unwrap()
            ),
            Error::<Test>::CannotRaiseDisputeOnApprovedMilestone
        );
        assert_noop!(
            Proposals::raise_dispute(
                RuntimeOrigin::signed(BOB),
                project_key,
                vec![submitted_milestone_key, 2u32].try_into().unwrap()
            ),
            Error::<Test>::CannotRaiseDisputeOnApprovedMilestone
        );
    })
...
#[test]
fn raise_dispute_with_single_jury_auto_completes() {
    build_test_externality().execute_with(|| {
        let contributions = get_contributions::<Test>(vec![BOB, CHARLIE], 1_000_000u128);
        let milestones = get_milestones(10);
        let jury = vec![JURY_1];
