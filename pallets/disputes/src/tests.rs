...
fn test_voting_on_a_dispute() {
    let mut jury_vec: BoundedVec<AccountIdOf<Test>, <mock::Test as pallet::Config>::MaxJurySize> =
        BoundedVec::new();
...
fn test_voting_on_a_dispute_which_does_not_exists() {
    ...
    new_test_ext().execute_with(|| {
        Dispute::<Test>::new(10, *ALICE, jury_vec, BoundedVec::default()).expect("Creation Failed");
        assert_noop!(
            PalletDisputes::vote_on_dispute(RuntimeOrigin::signed(*CHARLIE), 1, true),
...
fn test_extending_the_voting_from_a_non_jury_account() {
    ...
    new_test_ext().execute_with(|| {
        Dispute::<Test>::new(10, *ALICE, jury_vec, BoundedVec::default()).expect("Creation Failed");
        assert_noop!(
            PalletDisputes::extend_dispute(RuntimeOrigin::signed(*CHARLIE), 10),
            Error::<Test>::NotAJuryAccount
        );
    });
}
...
fn test_extending_the_voting_which_has_already_been_extended() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PalletDisputes::extend_dispute(RuntimeOrigin::signed(*BOB), 10),
