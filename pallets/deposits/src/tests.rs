...
fn take_deposit_takes_deposit() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::Native));
...
fn take_deposit_assert_last_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::Native));
        System::assert_last_event(mock::RuntimeEvent::Deposits(crate::Event::<Test>::DepositTaken(0, deposit)));
    });
}
...
fn take_deposit_unsupported_currency_type() {
    new_test_ext().execute_with(|| {
        let item = StorageItem::CrowdFund;
        assert_noop!(crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::KSM), Error::<Test>::UnsupportedCurrencyType);
    });
}
...
fn take_deposit_unsupported_storage_type() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::Native), Error::<Test>::UnsupportedStorageType);
    });
}
...
fn return_deposit_works() {
    new_test_ext().execute_with(|| {
        let item = StorageItem::CrowdFund;
        let alice_reserved_before = <Test as Config>::MultiCurrency::reserved_balance(CurrencyId::Native, &ALICE);
        let deposit_id = crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::Native).unwrap();
...
fn return_deposit_assert_event() {
    new_test_ext().execute_with(|| {
        ...
        let deposit_id = crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::Native).unwrap();
        assert_ok!(crate::Pallet::<Test>::return_deposit(deposit_id));
        System::assert_last_event(mock::RuntimeEvent::Deposits(crate::Event::<Test>::DepositReturned(0, deposit)));
    });
}
...
fn slash_deposit_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(<Test as Config>::MultiCurrency::free_balance(CurrencyId::Native, &slash_account), 0, "slash account should be empty to start");
        let item = StorageItem::CrowdFund;
        let deposit = MockDepositCalculator::calculate_deposit(item, CurrencyId::Native).unwrap();
        let alice_reserved_before = <Test as Config>::MultiCurrency::reserved_balance(CurrencyId::Native, &ALICE);
        let deposit_id = crate::Pallet::<Test>::take_deposit(*ALICE, item, CurrencyId::Native).unwrap();
        ...
        let alice_reserved_after = <Test as Config>::MultiCurrency::reserved_balance(CurrencyId::Native, &ALICE);
        let slash_account_balance_free = <Test as Config>::MultiCurrency::free_balance(CurrencyId::Native, &slash_account);
        assert_eq!(alice_reserved_after, alice_reserved_before, "Alice's reserve balance should have been emptied");
        assert_eq!(slash_account_balance_free, deposit, "slash_account's free balance should equal the deposit");
    });
}
...
fn slash_deposit_assert_event() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Deposits::slash_reserve_deposit(deposit_id));
        System::assert_last_event(mock::RuntimeEvent::Deposits(crate::Event::<Test>::DepositSlashed(0, deposit)));
    });
}
