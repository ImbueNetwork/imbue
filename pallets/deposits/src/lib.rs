...
pub mod pallet {
	...
	#[pallet::storage]
	pub type CurrentDeposits<T> = StorageMap<_, Blake2_128, <T as Config>::DepositId, Deposit<T>, OptionQuery>;
	...
	impl <T: Config> DepositHandler<BalanceOf<T>, AccountIdOf<T>> for Pallet<T> {
		...
		fn take_deposit(who: AccountIdOf<T>, deposit_id: Self::DepositId, item: Self::StorageItem, currency_id: T::CurrencyId) -> DispatchResult {
			ensure!(!CurrentDeposits::<T>::contains_key(&deposit_id), Error::<T>::DepositAlreadyExists);	
