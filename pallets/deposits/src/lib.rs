...
pub mod pallet {
    ...
    impl<T: Config> DepositHandler<BalanceOf<T>, AccountIdOf<T>> for Pallet<T> {
        ...
        fn take_deposit(
            ...
            let amount = <T as Config>::DepositCalculator::calculate_deposit(storage_item)?;
            ...
            let deposit = Deposit { who, amount };
        ...
        fn return_deposit(deposit_id: T::DepositId) -> DispatchResult {
            ...
            <T as Config>::Currency::unreserve(&deposit.who, deposit.amount);
