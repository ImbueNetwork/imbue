...
impl<T: Config> EnsureRole<AccountIdOf<T>, Role> for EnsureFellowshipRole<T> {
	...
	fn ensure_role(acc: &AccountIdOf<T>, role: Role, rank: Option<Rank>) -> Result<Self::Success, BadOrigin> {
	...
	fn ensure_role_in(acc: &AccountIdOf<T>, roles: Vec<Role>, ranks: Option<Vec<Rank>>) -> Result<Self::Success, BadOrigin> {
		let (actual_role, actual_rank) = Roles::<T>::get(acc).ok_or(BadOrigin)?;
