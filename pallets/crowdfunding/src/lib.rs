...
pub mod pallet {
    ...
    pub type BoundedContributions<T> = BoundedBTreeMap<
        AccountIdOf<T>,
        Contribution<BalanceOf<T>, BlockNumberFor<T>>,
        <T as Config>::MaxContributionsPerCrowdFund,
    >;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn update_crowdfund(
            ...
            ensure!(!crowdfund.cancelled, Error::<T>::CrowdFundCancelled);
        ...
        pub fn open_contributions(
            ...
            <T as Config>::AuthorityOrigin::ensure_origin(origin)?;
                ...
                CrowdFunds::<T>::contains_key(crowdfund_key),
                Error::<T>::CrowdFundDoesNotExist
            );
            ensure!(
                !CrowdFundsInRound::<T>::contains_key(crowdfund_key, RoundType::ContributionRound),
                Error::<T>::AlreadyInContributionRound
            );
    ...
    impl<T: Config> Pallet<T> {
        pub fn new_crowdfund(
            ...
            let crowdfund = CrowdFund {
                ...
                contributions: BTreeMap::new()
                    .try_into()
                    .expect("empty BTree is smaller than bound; qed"),
                required_funds,
                currency_id,
        ...
        pub fn do_update_crowdfund(
            ...
            if let Some(ms) = proposed_milestones {
                let total_percentage = ms.iter().fold(0, |acc: u32, ms: &ProposedMilestone| {
                    acc.saturating_add(ms.percentage_to_unlock)
                });
                ensure!(
                    total_percentage == 100,
        ...
        pub fn start_contribution_round(crowdfund_key: CrowdFundKey) -> DispatchResultWithPostInfo {
        ...
        pub fn new_contribution<'a>(
            ...
            if WhitelistSpots::<T>::contains_key(crowdfund_key) {
                let whitelist_spots =
                    Self::whitelist_spots(crowdfund_key).ok_or(Error::<T>::WhiteListNotFound)?;
                ensure!(
